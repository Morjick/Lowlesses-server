...
import { PlayerInterface, UserModel } from '../models/UserModel'
import { PlayerClassType, getClassForName } from '../data/game-classes/GameClass'
...
export class HeroesController {
  ...
  @OnMessage('by-hero')
  async byHeroe (@ConnectedSocket() socket: any, @MessageBody() stringMessage: any) {
    ...
    const userClass = user.userLockedData.classes.find((el) => el.name === playerClass.name)

    if (userClass.locked === undefined && userClass.locked === null) {
      return socket.emit('game-error', 'Ошибка при покупке персонажа: персонаж с таким именем не найден')
    }
    ...
    user.userLockedData.classes.find((el) => el.name === playerClass.name).locked = false

    await UserModel.update({ userLockedData: JSON.stringify(user.userLockedData) }, { where: { id: user.id } })
  ...
  @OnMessage('by-skill')
  async byClassesSkills (@ConnectedSocket() socket: any, @MessageBody() stringMessage: any) {
    ...
    if (userClass.locked) {
      socket.emit('game-error', 'Для покупки навыка необходимо сначала открыть персонажа')
      return
    }
    ...
    user.userLockedData.classes
      .find((el) => el.name === playerClass.name).skills
      .find((skill) => skill.name === message.skill)
      .locked = false
